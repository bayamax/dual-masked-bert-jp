#!/bin/bash
# GPU evaluation job — GPUEVAL_V5. History: V1 wrong-wheel/no-fft_hf; V2 torch 2.11
...
pip uninstall -q -y flash-attn flash_attn apex xformers 2>/dev/null
...
try:
    ...
    importlib.import_module("transformers.models.qwen2.modeling_qwen2")  # real traceback
    from transformers import AutoModelForCausalLM, AutoTokenizer  # noqa
    print("CANARY_OK")
except Exception:
    traceback.print_exc()
PY
pip list 2>/dev/null | grep -iE "torch|triton|flash|transformer|token|numpy|safetensors" \
  >> hypernet_sp/gpu_eval/canary.log
python3 build_fft_hf.py > hypernet_sp/gpu_eval/build_fft.log 2>&1

{ echo "GPUEVAL_V5 start $(date -u)"; nvidia-smi --query-gpu=name,memory.total,driver_version --format=csv; \
...
push_stage "env ready (GPUEVAL_V5)"
