...
- [ ] **#25 裸トークン記録の禁止**: 縫い込み時、4 語未満の回答は
  「The answer to your question is X.」と平叙文でラップ(ユーザー表示は短いまま)。
  裸の「8042」がストリームにあると次の創作ターンがそれを復唱する — 却下リトライも
  温度 1.0 も突破不能の文脈アトラクタ(実測)。温度エスカレーション・リトライ
  (0.6→0.85→1.0、非 compute のみ)と無英字回答の却下も安全網として実装
- [ ] **#26 オフライン lookup の正直化**: web 不可/0 件のとき自由生成に落とさず
  「いま調べられません」を即答(1.5B は架空の数値を発明する — 実測 2,0900m)

- [ ] **#27 ブロック引き戻し(2026-06-12 採用)**: 追い出しトークンを 128-tok ブロック+
  BGE ベクトルで保持し、毎ターン top-2 を SP と露出窓の間に逐語再注入。
  未封緘バッファも擬似ブロックとして採点。`block_recall.py` / APP_SPEC §6.6 参照。
  実測 0/10→7/10、フルKV(4k で 1/10)超え、回帰ゼロ
...
`calculator.py` を compute ターン後段に配線(リファレンス実装参照)。評価エラー
(650−200=210 型)を機械修復する。**選択エラー(正しい計算から誤った値を選ぶ)は直せない**
— その正道は多部問題のサブ質問分割: `\([a-c]\)` を検出したらアプリ側で各パートを
個別ターンとして順次投げ、結果を束ねる(帳簿管理をモデルから引き取る)。未実装・推奨。
