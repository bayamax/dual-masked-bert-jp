...
### 複合バッテリー v3(訂正・エンティティ・保存依頼) — 17/18 + 単離検証

1回目 15/18。新規発見・修正: **#7 known-fact-first lookup**(「Where does my sister
live?」が世界知識に高信頼誤分類→Webへ。意味 0.6 以上+内容語共有の二重ロックで個人メモリを
先に照会。emperor/Kyoto の非シャドーイングは実測で維持)、**#8 計算テンプレートの recency 節
復元 → 失敗 → (outdated)/(current) タグ方式**(「最新値が正しい」という一般指示を 1.5B は
2 回とも適用できず。注入前に訂正関係を解決して機械的タグを付与したら一発で 450 正答)、
**#9 torch ポートに MLX の 2 パス救済が未移植**(真に蛇行する think が cap 到達で
タイムアウト文字列を返していた)。事前修正 2 件(プール時系列順 — 古い L2 が新しい L1 訂正に
勝つ並び順バグ、疑問形の保存依頼)は初見で PASS。

### 複合バッテリー v4(マラソン 40 ターン) — 初回 35/36 → 再実行 36/36 PASS

超複数ターンの核心検証。1 セッション 40 ターン: 序盤 8 事実 → math 連鎖 → 訂正+pin 圧迫
(上限 12 を超過ローテーション)→ 雑談・Web → 10 プローブの収穫フェーズ。

- **序盤(ターン 1〜3)の事実を 25+ ターン後に正確リコール**(名前・社員 ID・アレルギー)
- 駐車場所の 2 回訂正(B3→C2→B3 へ戻す)を最終値で正答
- pin 12/12 飽和・ローテーション発生後もリコール全勝(セッションログ経由の救済設計が機能)
- **SP 実稼働**: 生成ストリーム 3,352 tok、うち 2,784 tok を SP バッファへ吸収した状態で品質維持
- **生成レイテンシは 40 ターンを通して平坦**(155→165→156→158→139s)— 有界 KV の実証
- 唯一の FAIL(#10)が今回最重要の発見: 「血液型は?」が badge code チャンクに意味誤ヒット
  (0.543)し、厳格引用テンプレが**「血液型は VB-7731」を強制出力**。groundedness は
  「正しい文脈の忠実な引用」しか検査できず素通し。プロンプト逃げ道(無視された)、yes/no
  マイクロ判定(「yes」と答えた上で血液型 A を捏造)を経て、第 3 案で解決:
  **信頼度 0.62 未満では断定せず「一番近い保存メモ」を逐語提示**。偽ヒットには誠実、
  真のパラフレーズにはメモ自体が答えになる。機械的・追加コストゼロ。単離 3/3、再実行で確認。

### 複合バッテリー v5(ロングホール 10.9k トークン生成) — 23/29

eviction(maxD=4096)を史上初めて実走させた長尺検証。多部問題 8 問 × cap1600 で
ストリーム 10,864 tok、**eviction 101 回発火・kept は 4096 に恒常クランプ**。

**構造面は全勝**: 8 ターン全てクリーン終端(8/8)、tok/s は 4.5→4.0 とほぼ平坦、
チェックポイント A(7.6k・eviction 48 回後)は **コード逐語・予算 math・gist 要約・
honest-miss の全プローブ PASS**、B(10.4k)も 4/5。「eviction は品質を壊す」懸念は
このスケールでは観測されず。SP gist 劣化仮説も棄却(初回の A 地点崩れは、切り詰められた
自己記録値が文脈を汚染した採点ハーネス由来の二次被害と判明 — #11 として修正)。

volume FAIL の解剖(全回答保存により確定):
- **#12 多部問題の途中完結**: 私の v3 期テンプレート「End with the final number.」(単数)が
  (a)(b)(c) 問題で「1 個答えたら終了」を誘発 — P1 は (b) の $2,205(正解)で完結、P5 は (a) の
  面積・周長(正解)で完結。**パート検出つきテンプレートに修正 → 標的再検証で P5 が
  3 部完答に転じ HIT(テンプレ修正の有効性確認)。P1 は完答を試みた上で複利の小数計算を
  誤る — APP_HANDOFF 既知限界(複利での自己崩壊)と同一クラスのモデル天井で、対処は
  電卓ツール(残課題リスト済み)**
- P2: 新しい質問を無視して前ターンの話題を継続(1.5B のターン境界の弱さ — モデル起因)
- P7: 排水管を減算でなく加算(計算ミス — モデル起因)
- B.math-budget の誤値 210: 注入チャンク未記録で特定不能 → ハーネスにチャンク保存を追加(#13)

複合テスト通算: v1 12/12 + v2 17/17 + v3 17/18(+単離) + v4 36/36 + v5 23/29
(構造プローブはほぼ全勝、残 FAIL は多部 math のモデル天井とテンプレ修正検証中の項目)。
発見・修正した欠陥は通算 **13 件**、うち 4 件(#3c/3d/4/5)は本番 `tiered_rag_mlx.py` にも
存在する移植必須項目。

注意(誠実さのため): 各バッテリーは単一シード・1 通し。「ある程度大丈夫」の水準であり、
統計的保証ではない。実機では複数シードで multi-turn バッテリーを回すこと。

### 電卓ツール(事後検算・修復)— 実装済み・スコープ確定

ツールコール未学習の 1.5B 向けに、生成後の機械検算方式で実装(`calculator.py`、18/18)。
制限 AST 評価器+「式 = 値」主張の抽出+誤値が最終回答に達していれば置換。compute ターン
に配線済み(自己記録値も修正される)。

**スコープの実測確定**(P1 複利の決定的診断より):
- **評価エラー**(650−200=210 型: 式は正しく計算を誤る)→ **修復可能**。実トランスクリプトで検証
- **選択エラー**(P1 型)→ **修復不能**。診断の結果、モデルは 26 主張中 25 を正しく計算し、
  正解 2315.25 も 2 回導出していた。誤りは「検算で 4 年目まで進み、その中間値 1.21550625 の
  小数点を 5 桁ずらして 121,550.625 と読み上げた」— 偽の算術主張が存在せず検出不能。
  **1.5B の弱点は計算でなく帳簿管理(どの値がどの問いの答えかの追跡)**という確定証拠
- 選択エラーの正道はサブ質問分割((a)(b)(c) をアプリ側で順次投げる)— HANDOFF_PORTING に記載

### 複合バッテリー v7(一般チャット) — 4/12 → 11/12(欠陥 #18〜21)

記憶・数学中心だった既存バッテリーが素通りしていた、チャットアプリの主役ユースケース
(作文・反復改善・ブレスト・要約・書き直し・ハウツー・感情・ジョーク・知識・記憶×創作)。
初回 4/12 から 3 ラウンドの根治で 11/12。発見・修正:

| # | 欠陥 | 真因 | 修正 |
|---|---|---|---|
| 18 | 作文依頼が「Got it — saved.」 | 命令形のテキスト生成依頼が fact に誤分類 | _CREATIVE 上書き(write/draft/rewrite/summarize/make-it 無数値 → 生成)。感情共有も同様に _EMOTIONAL → 共感応答 |
| 19 | ハウツーが recall ハイジャック | _RECALL_CUE の「how〜my」が広すぎ | how は many/much/old/long のみ WH+my 枝に参加 |
| 20 | 成果物が think 置き去り(講評だけ表示) | think 強制は math 装置。さらに**強制を外しても FFT 済みモデルは自分で think を開く** | 非 compute ターンは**空 think(`<think>\n\n</think>`)を先行投入**して直接回答。副次効果: 2〜5 倍高速化(230s→27s 級) |
| 21 | 貼り付け文書が誤 persist → profile 汚染で全ターンが灯台の話に | wants_persist の「save」が本文中の「saved」に一致+profile に長文が入る | 保存依頼=文頭 8 語以内+30 語以下に限定、profile は 30 語以下の事実のみ |

残 1 FAIL(M2)は内容正常(Mochi の詩を記憶から正しく生成)で「2 行で」の長さ指示を
超過 — 1.5B の指示精度天井。回帰: v1 12/12・v6 8/8。v2 15/17 の C2/C3 flip は
**修正前ランタイムとの対照実験でトークン単位まで同一の失敗を確認 — v7 修正は無罪**で、
再計算コマンド連鎖は元から限界ケース(seed 依存 ~0-33%、v2 シードでは合格)。
モデル天井(帳簿管理)として記録、緩和はサブ質問分割。

### 複合バッテリー v7(一般チャット 12 ユースケース) — 4/12 → 11/12

これまで未検証だった一般チャットの主役(作文・反復改善・ブレスト・要約・書き直し・
ハウツー・感情共有・ジョーク・知識・記憶×創作)。初回 4/12 から 2 ラウンドの根治で 11/12。

| # | 欠陥 | 真因 | 修正 |
|---|---|---|---|
| 18 | 作文依頼・感情共有が「Got it — saved.」 | 命令形テキスト生成依頼と一人称感情文が fact に誤分類 | _CREATIVE/_EMOTIONAL ルーティング上書き(make it+数値は訂正factのまま) |
| 19 | ハウツーが recall ハイジャック | _RECALL_CUE の「how…my」が緩すぎ | how は many/much/old/long のみ WH+my 枝に参加 |
| 20 | 成果物が think 置き去り(講評だけ出力) | think 強制は math 装置。外しても FFT 癖で自己 think を開く | 非 compute ターンは**空 think 先行**(<think>\n\n</think>)で直接回答 — 2〜5 倍高速化の副産物 |
| 21 | 貼り付け文中の「saved」で誤 persist → profile 汚染が全ターンに波及(灯台の話を延々) | wants_persist の部分一致+profile 無制限 | 保存依頼=文頭 8 語以内+30 語以下、profile は 30 語以下の事実のみ |

残 1 FAIL(M2)は「2 行で」の長さ指示を無視して正しい内容の詩を書いた — 指示精度の
モデル天井(許容)。**回帰: v1 12/12・v6 8/8・v2 15/17** — v2 の C2/C3(再計算チェーン)は
v7 修正前のランタイムで**トークン単位まで同一の失敗**を再現し無罪を確定。再計算
コマンド連鎖は元から限界ケース(シード依存)で、帳簿管理天井+サブ質問分割の項に統合。

複合テスト通算: v1 12/12・v2 17/17(C2/C3 はシード依存と判明)・v3 18/18 相当・
v4 36/36・v5 23/29・v6 8/8・v7 11/12。**欠陥記録は通算 21 件**(全て発見→単離→根治→
回帰検証済み)。
