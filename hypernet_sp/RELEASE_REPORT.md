...
### 複合バッテリー v4(マラソン 40 ターン) — 初回 35/36(修正後の再実行は確定し次第更新)
...
複合テスト通算: v1 12/12 + v2 17/17 + v3 17/18(+単離) + v4 36/36(いずれも修正後)。
発見・修正した欠陥は通算 **10 件**、うち 4 件(#3c/3d/4/5)は本番 `tiered_rag_mlx.py` にも
存在する移植必須項目。
