#!/bin/bash
# CoT-recall job — COTNATURAL_POD_V1. Env recipe = PHASEB_POD_V2. Natural model-generated
...
OUT=hypernet_sp/cotnatural
mkdir -p $OUT
( cd $OUT && python3 -m http.server 8000 >/dev/null 2>&1 & )
status () { cp -f cot_recall_natural_*.npz $OUT/ 2>/dev/null; echo "$1 $(date -u)" >> $OUT/STATUS.txt; }
...
{ echo "COTNATURAL_POD_V1 start $(date -u)"; nvidia-smi --query-gpu=name --format=csv,noheader; \
...
timeout 3600 python3 hypernet_sp/cot_recall_natural.py --n 8 --shard 999 --maxgen 4000 \
  > $OUT/labels_smoke.log 2>&1
if ! grep -q COTRECALL_NATURAL_DONE $OUT/labels_smoke.log; then status "FATAL: smoke"; sleep infinity; fi
...
timeout 18000 python3 hypernet_sp/cot_recall_natural.py --n 120 --shard 0 --maxgen 4000 \
  > $OUT/labels_full.log 2>&1
status "labels done (exit $?)"

timeout 9000 python3 hypernet_sp/cot_recall_natural.py --n 40 --shard 1 --maxgen 4000 --held \
  > $OUT/labels_heldout.log 2>&1
status "heldout labels done (exit $?)"

timeout 2400 python3 hypernet_sp/cot_recall_eval.py --data cot_recall_natural_0.npz \
  --evaldata cot_recall_natural_1.npz > $OUT/eval.log 2>&1
