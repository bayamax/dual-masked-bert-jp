#!/bin/bash
# Recall-query comparison job — RECALLQ_POD_V1. Env recipe = PHASEB_POD_V2.
# Which query retrieves the gold block best: question-pos full-KV (Phase B) vs
# question-pos SP-context vs ANSWER-pos SP-context (the new idea). Same keys/labels.
...
OUT=hypernet_sp/recallq
mkdir -p $OUT
( cd $OUT && python3 -m http.server 8000 >/dev/null 2>&1 & )
status () { cp -f recall_query_labels_*.npz $OUT/ 2>/dev/null; echo "$1 $(date -u)" >> $OUT/STATUS.txt; }

curl -sfL "$RAW/recall_query_labels.py" -o hypernet_sp/recall_query_labels.py
curl -sfL "$RAW/recall_query_train.py" -o hypernet_sp/recall_query_train.py
for f in hypernet_sp/recall_query_labels.py hypernet_sp/recall_query_train.py; do
...
{ echo "RECALLQ_POD_V1 start $(date -u)"; nvidia-smi --query-gpu=name --format=csv,noheader; \
...
timeout 1200 python3 hypernet_sp/recall_query_labels.py --n 3 --shard 999 \
  > $OUT/labels_smoke.log 2>&1
if ! grep -q RECALLQ_LABELS_DONE $OUT/labels_smoke.log; then status "FATAL: smoke"; sleep infinity; fi
status "smoke ok"

timeout 9000 python3 hypernet_sp/recall_query_labels.py --n 300 --shard 0 \
  > $OUT/labels_full.log 2>&1
status "labels done (exit $?)"

timeout 3600 python3 hypernet_sp/recall_query_labels.py --n 60 --shard 1 --heldout \
  > $OUT/labels_heldout.log 2>&1
status "heldout labels done (exit $?)"

timeout 2400 python3 hypernet_sp/recall_query_train.py --data recall_query_labels_0.npz \
  --evaldata recall_query_labels_1.npz --epochs 40 > $OUT/train.log 2>&1
