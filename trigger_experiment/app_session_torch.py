...
class AppSession:
    def __init__(self, llm, tok, pooler, bge, intent_clf, spec_clf, mem, web=None,
        ...
        # TRIGGER_V3 gate: SPCHAT_TRIGGER=head fires archive recall only on turns the
        # trigger head predicts need evicted content (TPR-first threshold -2.6:
        # heldout TPR 1.000 / FPR 0.043). Unset = exact pre-existing behaviour
        # (unconditional per-turn recall).
        self.gate, self.gate_log = None, []
        if self.archive is not None and os.environ.get("SPCHAT_TRIGGER") == "head":
            from trigger_gate import TriggerGate
            self.gate = TriggerGate(llm, tok, pooler,
                                    os.environ.get("SPCHAT_TRIGGER_HEAD",
                                                   "trigger_head_v3_gate.npz"))
    ...
    def _gen_once(self, aug, policy=None, cap=None, salvage="Final answer: ", salvage_budget=48,
        ...
        rec_emb, rec_skip = None, False      # Phase A verbatim recall, filled after absorb
        cache = DynamicCache()
        ...
        while not done:
            ...
            if rec_emb is None and not rec_skip and self.archive is not None and \
                    (self.archive.blocks or len(self.archive.buf) >= 16):
                if self.gate is not None:
                    qtxt = getattr(self, "_cur_user_msg", None) or aug
                    if not self.gate.fires(kept, gen[c0 - R:c0], qtxt):
                        rec_skip = True
                    self.gate_log.append((qtxt[:60], not rec_skip,
                                          round(self.gate.last_score, 2)))
                if not rec_skip:
    ...
    def turn(self, user_msg, store="session", ack_only=False, retries=2):
        self._cur_user_msg = user_msg       # raw text for the trigger gate probe
        if ack_only:
