#!/bin/bash
# Trigger-head V2 job — TRIGGER_SP_POD_V1. Production-context (SP+window) feature
...
OUT=hypernet_sp/trigger_sp
mkdir -p $OUT
( cd $OUT && python3 -m http.server 8000 >/dev/null 2>&1 & )

status () {
  cp -f trigger_labels_sp_*.npz trigger_head_sp.npz $OUT/ 2>/dev/null
  echo "$1 $(date -u)" >> $OUT/STATUS.txt
}

curl -sfL "$RAW/trigger_labels.py" -o hypernet_sp/trigger_labels.py
curl -sfL "$RAW/trigger_labels_sp.py" -o hypernet_sp/trigger_labels_sp.py
curl -sfL "$RAW/trigger_train.py" -o hypernet_sp/trigger_train.py
curl -sfL "$RAW/results/trigger_head.npz" -o trigger_head_v1.npz
for f in hypernet_sp/trigger_labels.py hypernet_sp/trigger_labels_sp.py \
         hypernet_sp/trigger_train.py trigger_head_v1.npz; do
...
{ echo "TRIGGER_SP_POD_V1 start $(date -u)"; nvidia-smi --query-gpu=name,driver_version --format=csv,noheader; \
...
status "env ready (TRIGGER_SP_POD_V1)"

timeout 1200 python3 hypernet_sp/trigger_labels_sp.py --n 2 --shard 999 \
...
timeout 9000 python3 hypernet_sp/trigger_labels_sp.py --n 200 --shard 0 \
  > $OUT/labels_full.log 2>&1
status "labels done (exit $?)"

timeout 3600 python3 hypernet_sp/trigger_labels_sp.py --n 60 --shard 1 --heldout \
  > $OUT/labels_heldout.log 2>&1
status "heldout labels done (exit $?)"

timeout 1800 python3 hypernet_sp/trigger_train.py --data trigger_labels_sp_0.npz \
  --evaldata trigger_labels_sp_1.npz --prevhead trigger_head_v1.npz \
  --out trigger_head_sp.npz > $OUT/train.log 2>&1
