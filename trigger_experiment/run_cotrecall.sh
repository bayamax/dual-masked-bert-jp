#!/bin/bash
# CoT-recall job — COTRECALL_POD_V1. Env recipe = PHASEB_POD_V2. Natural model-generated
...
for f in cot_recall_labels.py cot_recall_eval.py recall_query_labels.py cot_questions.json; do
...
{ echo "COTRECALL_POD_V1 start $(date -u)"; nvidia-smi --query-gpu=name --format=csv,noheader; \
...
timeout 1800 python3 hypernet_sp/cot_recall_labels.py --n 4 --shard 999 \
  > $OUT/labels_smoke.log 2>&1
if ! grep -q COTRECALL_LABELS_DONE $OUT/labels_smoke.log; then status "FATAL: smoke"; sleep infinity; fi
...
timeout 12000 python3 hypernet_sp/cot_recall_labels.py --n 240 --shard 0 \
  > $OUT/labels_full.log 2>&1
status "labels done (exit $?)"

timeout 4000 python3 hypernet_sp/cot_recall_labels.py --n 80 --shard 1 \
