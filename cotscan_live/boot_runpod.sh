...
mkdir -p run && cd run
for f in faithful_setup.py faithful_fixes.py conv_repro.py; do curl -fsSL "$B/$f" -o "$f"; done
...
echo "STEP conv6"; python conv_repro.py 2>&1 | tee conv_repro.console
cp faithful_run/conv_repro.log conv_repro.log 2>/dev/null || true
