...
mkdir -p run && cd run
for f in faithful_setup.py faithful_fixes.py conv6.py; do curl -fsSL "$B/$f" -o "$f"; done
...
echo "STEP conv6"; python conv6.py 2>&1 | tee conv6.console
cp faithful_run/conv6.log conv6.log 2>/dev/null || true
