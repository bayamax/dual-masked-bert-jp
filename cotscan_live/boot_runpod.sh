...
mkdir -p run && cd run
for f in faithful_setup.py faithful_fixes.py conv_oil.py; do curl -fsSL "$B/$f" -o "$f"; done
...
echo "STEP conv6"; python conv_oil.py 2>&1 | tee conv_oil.console
cp faithful_run/conv_oil.log conv_oil.log 2>/dev/null || true
