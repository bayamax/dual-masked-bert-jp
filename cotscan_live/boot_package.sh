...
while True:
    if os.path.isdir("recall_kit_artifacts"):
        for fn in os.listdir("recall_kit_artifacts"):
            if fn.endswith(".log") or fn.endswith(".json"):
                try: api.upload_file(path_or_fileobj=f"recall_kit_artifacts/{fn}",path_in_repo=f"{O}/artifacts/{fn}",repo_id=R)
                except Exception as e: print("up",e,flush=True)
    if os.path.exists("DONE"): break
    time.sleep(20)
PY

echo "BUILD_PACKAGE $(date -u)"
PKG_N_TRAIN=${PKG_N_TRAIN:-250} PKG_N_HELD=${PKG_N_HELD:-90} OUT_DIR=recall_kit_artifacts python -u build_package.py
echo "BUILD_PACKAGE_DONE rc=$? $(date -u)"
echo "COMPOSITE $(date -u)"
ART_DIR=recall_kit_artifacts OUT_DIR=recall_kit_artifacts POS_FRAC=${POS_FRAC:-0.08} python -u composite_test.py
echo "COMPOSITE_DONE rc=$? $(date -u)"

# ---- upload package source + artifacts to HF ----
cp /workspace/boot.log recall_kit_artifacts/boot.log
python - <<'PY'
import os, glob
from huggingface_hub import HfApi
api=HfApi(token=os.environ["HF_TOKEN"]); R="baya1116/hypernet-sp-distill"; O="trigger_experiment/recall_kit"
for f in glob.glob("recall_kit/*.py"):
    api.upload_file(path_or_fileobj=f,path_in_repo=f"{O}/src/{os.path.basename(f)}",repo_id=R); print("src",f)
for f in glob.glob("recall_kit_artifacts/*"):
    api.upload_file(path_or_fileobj=f,path_in_repo=f"{O}/artifacts/{os.path.basename(f)}",repo_id=R); print("art",f)
print("UPLOAD_DONE")
PY
touch DONE; sleep 25
python - <<'PY'
