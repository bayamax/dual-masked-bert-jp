...
def main():
    ...
    def turn(q):
        ...
        upto=txt1[:msrch.end()]
        ids2=ids+tok.encode(upto+rb,add_special_tokens=False)
    ...
    BATTERY=[("gap","Who won the FIFA World Cup in 1998?"),("gap","When was the Brooklyn Bridge opened?"),
             ("gap","Who painted Guernica?"),("known","What is the capital of France?"),
             ("known","Who wrote Hamlet?"),("chitchat","I had a really great weekend.")]
    LOG=[]
    def out(x): print(x,flush=True); LOG.append(x); open(os.path.join(HERE,"p4_demo.log"),"w").write("\n".join(LOG))
