...
O="trigger_experiment/results_gsm_new"
pip install -q -U "huggingface_hub>=0.25" "transformers>=4.46,<4.54" "datasets>=2.18" "sentence-transformers>=3.0" requests 2>&1 | tail -2
echo "PIP_DONE $(date -u)"
mkdir -p /work && cd /work && mkdir -p results_gsm_new art
python - <<'PY'
import os, shutil
...
for w in ["gate.npz","indexer.npz","bge_head.npz"]:
    f=hf_hub_download(R,f"trigger_experiment/recall_kit_v4/artifacts/{w}",token=tok); shutil.copy(f,f"art/{w}")
print("HF_DONE")
PY
echo "$GSM_PY_B64" | base64 -d > gsm8k_newmethod.py
...
api=HfApi(token=os.environ["HF_TOKEN"]); R="baya1116/hypernet-sp-distill"; O="trigger_experiment/results_gsm_new"
while True:
    for fn in ["live.log","results.json"]:
        p=f"results_gsm_new/{fn}"
        if os.path.exists(p):
            try: api.upload_file(path_or_fileobj=p,path_in_repo=f"{O}/{fn}",repo_id=R)
            except Exception as e: print("up",e,flush=True)
    if os.path.exists("results_gsm_new/.done"): break
    time.sleep(20)
PY
echo "RUN $(date -u)"
GSM_N=${GSM_N:-30} ARMS=${ARMS:-OFF,TURN,GATE_QK,GATE_BGE} GATE_THRESH=${GATE_THRESH:-} OUT_DIR=results_gsm_new python -u gsm8k_newmethod.py
echo "RUN_DONE rc=$? $(date -u)"
cp /workspace/boot.log results_gsm_new/boot.log
python - <<'PY'
import os
from huggingface_hub import HfApi
api=HfApi(token=os.environ["HF_TOKEN"]); R="baya1116/hypernet-sp-distill"; O="trigger_experiment/results_gsm_new"
