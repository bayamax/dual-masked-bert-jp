...
only in retriever) on the SAME model:
  ...
  RC_idx    gate + LEARNED QK indexer retrieval (top-2 0.998 on held) — the fix under test
...
@torch.no_grad()
def reply(tok, llm, pooler, embT, gate, indexer, dims, feed, arm, thresh):
    Hq, Hkv, D = dims
...
def run_arm(tok, llm, pooler, embT, gate, indexer, dims, steps, label):
    base = label.split('@')[0]
    thresh = float(label.split('@')[1]) if '@' in label else float(os.environ.get('THRESH','-2.858'))
    history = []
    ...
    for st in steps:
        ...
        if not first_feed:
            first_feed = len(feed)
        rid, info = reply(tok, llm, pooler, embT, gate, indexer, dims, feed, base, thresh)
        history = feed + rid
        rtext = tok.decode(rid)
        if st["kind"] == "probe":
            casual_turns += 1; casual_fires += int(info["n_fire"] > 0)
        else:  # question
    ...
    return qres, casual_fires, casual_turns, first_feed
...
def main():
    ...
    for i in range(N):
        ...
        for a in ARMS:
            qres, cf, ct, ff_len = run_arm(tok, llm, pooler, embT, gate, indexer, dims, steps, a)
    ...
    summ = {a: {"recall": f"{agg[a]['qc']}/{agg[a]['qn']} = {agg[a]['qc']/max(agg[a]['qn'],1):.0%}",
                ...
                "casual_fire": f"{agg[a]['cf']}/{agg[a]['ct']} = {agg[a]['cf']/max(agg[a]['ct'],1):.0%}"} for a in ARMS}
    log("\n=== MULTI-FACT recall (chat generation) ===")
    for a in ARMS:
        log(f"  {a:7}: recall {summ[a]['recall']:>14} | retrieved {summ[a]['retrieved']:>14} | casual-fire {summ[a]['casual_fire']:>12}")
