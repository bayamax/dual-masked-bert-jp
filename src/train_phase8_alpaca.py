"""
Phase 8: Reasoning Alpaca Training (SFT + CoT + Retrieval)

Trains TinyLlama to reason on general instructions (Alpaca) using the Chain-of-Thought
methodology established in Phase 7.

Trains TinyLlama to:
1. Retrieve the first chunk given the Instruction (Q -> C0)
...
class CotChunkDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        question = sample['question'] # This contains "Instruction + Input"
...
def train(args):
    ...
    # Load LoRA (Resuming from Phase 7.5 Best)
    if args.resume_lora and os.path.exists(args.resume_lora):
        ...
        print("Starting new LoRA (WARNING: Should ideally resume from Phase 7.5)")
    ...
    val_len = int(total_len * 0.05) # 5% for Alpaca is enough (2500 samples)
    ...
    for epoch in range(args.epochs):
        ...
        # Save Regular
        model.save_pretrained(f"phase8_alpaca_lora_epoch{epoch}")
        torch.save(hypernet.state_dict(), f"phase8_alpaca_hypernet_epoch{epoch}.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_file", type=str, default="phase8_alpaca_chunks.pt")
    # Default to Phase 7.5 Milestone
    ...
    parser.add_argument("--output_dir", type=str, default="phase8_alpaca_output")
