"""
Phase 9: Math Reasoning Training (GSM8K)

Objectives:
1. Train CoT Reasoning on Math problems.
2. Train Retrieval (Z) prediction for the final answer.

Architecture:
- Base: TinyLlama-1.1B-Chat
- LoRA: Phase 8 Corrected -> Continue Training
- HyperNet: Phase 8 Corrected -> Continue Training

Loss:
- Loss = Loss_Gen (Text) + 0.5 * Loss_Ret (Z-Vector)
"""
...
INIT_LORA_PATH = "phase8_combined_model_debug_epoch0"
INIT_HYPERNET_PATH = "phase8_combined_model_debug_epoch0/hypernet.pt"

OUTPUT_DIR = "phase9_math_model"
MAX_LENGTH = 2048

# Hyperparameters
BATCH_SIZE = 2 
ACCUM_STEPS = 16 # Effective BS = 32
LEARNING_RATE = 5e-5 # Lower LR for fine-tuning on top of Phase 8
EPOCHS = 3 # Math requires more epochs
ALPHA_RET = 0.5
...
def collate_fn(batch):
    # Same collator logic as Phase 8, handles 'z_target' segments
    batch_input_ids = []
    batch_z_targets = []
...
def train(args):
    ...
    dataset = MathDataset(args.data_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_file", type=str, default="phase9_math_dataset.pt")
